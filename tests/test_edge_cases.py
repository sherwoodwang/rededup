...
class EdgeCaseTest(unittest.TestCase):
    ...
    def test_hash_collision_handling(self):
        """Repository handles hash collisions correctly using custom weak hash."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repository_path = Path(tmpdir) / 'repository'
            repository_path.mkdir()

            # Create files that will have XOR hash collision
            (repository_path / 'file1').write_bytes(b'\x00\x00\x00\x00\x01\x01\x01\x01')
            (repository_path / 'file2').write_bytes(b'\x01\x01\x01\x01\x00\x00\x00\x00')

            with Processor() as processor:
                with Repository(processor, str(repository_path), create=True) as repository:
                    # Use weak XOR hash to force collisions
                    repository._hash_algorithms['xor'] = (4, compute_xor)
                    repository._default_hash_algorithm = 'xor'
    ...
    def test_empty_files(self):
        """Repository correctly handles zero-byte files."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repository_path = Path(tmpdir) / 'repository'
            repository_path.mkdir()

            # Create empty files
            (repository_path / 'empty1.txt').write_bytes(b'')
            (repository_path / 'empty2.txt').write_bytes(b'')
            (repository_path / 'nonempty.txt').write_bytes(b'content')
    ...
    def test_large_directory(self):
        """Repository handles directory with many files efficiently."""
        with tempfile.TemporaryDirectory() as tmpdir:
            repository_path = Path(tmpdir) / 'repository'
            repository_path.mkdir()

            large_dir = repository_path / 'many_files'
            large_dir.mkdir()
